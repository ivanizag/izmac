...
volume and reading it, is the driver's work and not the ROM's.

So a bootable disk is the driver descriptor map, a partition map, a driver,
and the volume after them. The layout below is the one Apple's HD SC Setup
writes and the one the drivers expect:

	block 0        driver descriptor map
	blocks 1-63    partition map, three entries used of the 63 reserved
	blocks 64-95   the driver
...
The driver is not built here either. It is a real Macintosh driver, lifted
...
const (
	...
	// three, and the driver the 32 it is given there too.
	partitionMapStart  = 1
	partitionMapBlocks = 63
	driverStart        = 64
	driverBlocks       = 32
	volumeStart        = driverStart + driverBlocks
)

/*
The status words Apple's formatter writes. They are flags the driver reads
and there is nothing to work out about them, so they are copied as they are
found on a working disk.
*/
const (
...
// Driver is the SCSI driver taken off a disk that already boots, along with
// the partition map entry that describes it
type Driver struct {
	// code is the driver itself, a whole number of blocks
	code []uint8

	/*
		entry is the whole block the donor's partition map spends on the
		driver, carried over as it is rather than built again. Past the
		fields Inside Macintosh documents there is more in there, a table
		the driver reads to find itself, and since the driver being
		...
		fields nobody here understands, and a driver that loads and then
		stops without a word.
	*/
	entry []uint8

	// Processor is the cpu the driver is for, '68000' on the disks a Plus
	// can use. Read out of the entry for the messages.
	Processor string
...
// Blocks is the size of the driver, which is what the driver descriptor and
// the partition map entry both count in
func (d *Driver) Blocks() uint32 {
	return uint32(len(d.code) / BlockSize)
}

/*
ReadDriver takes the SCSI driver out of a partitioned disk image. Any disk
...
*/
func ReadDriver(filename string) (*Driver, error) {
	...
	return readDriverAt(file, filename)
}

/*
readDriverAt is ReadDriver over anything that reads, so that an image can be
looked at before it is a file. Only the blocks the driver is in are touched,
which is why a few blocks off the front of a disk are as good as the whole of
one.
*/
func readDriverAt(r io.ReaderAt, filename string) (*Driver, error) {
	...
	if binary.BigEndian.Uint16(block0) != driverDescriptorSignature {
		return nil, fmt.Errorf("%v does not start with a driver descriptor map, "+
			"so it has no driver to take", filename)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("can not read the driver of %v at block %v: %w",
			filename, start, err)
	}
...
/*
readEntry walks the partition map for the entry that covers the driver and
keeps the whole block.
*/
func (d *Driver) readEntry(r io.ReaderAt, driverBlock uint32) error {
	entry := make([]uint8, BlockSize)
	_, err := r.ReadAt(entry, partitionMapStart*BlockSize)
	if err != nil {
		return fmt.Errorf("can not read the partition map of %v: %w", d.source, err)
	}
	if binary.BigEndian.Uint16(entry) != partitionSignature {
		return fmt.Errorf("%v has a driver but no partition map", d.source)
	}
	...
	for i := uint32(0); i < entries; i++ {
		...
		if binary.BigEndian.Uint32(entry[8:]) != driverBlock {
			continue
		}

		d.entry = entry
		d.Processor = strings.TrimRight(string(entry[120:136]), "\x00")
		return nil
	}

	return fmt.Errorf("%v has a driver at block %v that no partition map entry covers",
		d.source, driverBlock)
}
...
func bareVolumeError(filename string) error {
	return fmt.Errorf("%v is a bare HFS volume, with no partition map and no "+
		"driver on it. Images like this one are made for the emulators that patch "+
		"the ROM to supply a driver of their own, and the ROM izmac runs boots by "+
		"loading one off the disk. Name a disk that has one and izmac makes up the "+
		"rest as it goes, writing to neither:\n"+
		"    izmac -driver <a disk that boots> %v",
		filename, filename)
}

/*
buildHeader lays out the driver descriptor map, the partition map and the
driver, which is everything that comes before the volume on a disk the ROM
can boot. It is the same bytes whether they are written to an image or kept
in memory in front of one.
*/
func buildHeader(driver *Driver, volumeBlocks uint32) []uint8 {
	...
	binary.BigEndian.PutUint16(header[16:], 1) // one driver follows
	binary.BigEndian.PutUint32(header[18:], driverStart)
	binary.BigEndian.PutUint16(header[22:], uint16(driver.Blocks()))
	...
	entries := []struct {
		...
		{"", "", driverStart, driverBlocks, 0}, // the donor's, copied below
	...
	copy(header[driverStart*BlockSize:], driver.code)
